...
module.exports.createCompact = function(options) {
  ...
  function copyFile(src, callback) {
    ...
    var writer = fs.createWriteStream(options.destPath + '/' + hash + '-' + path.basename(src));
    reader.pipe(writer);
  ...
  function middleware() {
    ...
    return function(req, res, next) {
      processNamespaceGroups(namespaceGroups, function(error, results) {
        ...
        var compactJsFn = function () {
          return results;
        };
        
        var compactJsHtmlFn = function() {
          return results.map(function(filename) {
            return '<script src="' + filename + version + '"></script>';
          }).join('');
        };
        ...
        if (typeof res.locals === 'function') {
          // Express 3.x
          res.locals({
            compactJs: compactJsFn,
            compactJsHtml: compactJsHtmlFn
          });
        }
        else {
          // Express 4.x
          res.locals.compactJs = compactJsFn;
          res.locals.compactJsHtml = compactJsHtmlFn;
        }
